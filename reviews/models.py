...
alphanumeric = RegexValidator(
    r"^[0-9a-zA-Z\s]*$", "Only alphanumeric characters are allowed."
)
...
class Company(models.Model):
    ...
    def get_api_url(self, request=None):
        return api_reverse(
            "api_v1:company-rud", kwargs={"pk": self.pk}, request=request
        )
...
class Product(models.Model):
    ...
    def get_api_url(self, request=None):
        return api_reverse(
            "api_v1:product-rud", kwargs={"pk": self.pk}, request=request
        )
...
class Review(models.Model):
    ...
    comment = models.CharField(
        "Review comment", max_length=128, validators=[alphanumeric]
    )
    stars = models.IntegerField(
        "Number of stars", validators=[numeric, min_stars, max_stars]
    )
